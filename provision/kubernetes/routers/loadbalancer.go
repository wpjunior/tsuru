...
type KubeRouter interface {
	CreateRouter(opts provision.RouterOptions) error
	UpdateRouter(opts provision.RouterOptions) error
}
...
func (s *LBService) UpdateRouter(opts provision.RouterOptions) error {
	return nil
}

func (s *LBService) CreateRouter(opts provision.RouterOptions) error {
	appName := opts.App.GetName()
	lbService, err := s.getLBService(opts.App)
	if err != nil {
		if !k8sErrors.IsNotFound(err) {
			return err
		}
	...
	// TODO: understand beter this case
	// // if opts == nil {
	...
	webService, err := s.getWebService(appName, lbService.Labels)
	if err != nil {
		// TODO: test this one
...
func (s *LBService) fillLabelsAndAnnotations(svc *v1.Service, app appTypes.App, webService *v1.Service) error {
	// TODO: read more about extraData
